...
<body id="body">
  ...
  <section id="topbar" class="d-none d-lg-block">
    <div class="container clearfix">
      <?php         
        $stmt = $DB_con->prepare('SELECT * FROM contato_mapa WHERE contato_id_user = 58 ORDER BY contato_id ASC LIMIT 1');
        $stmt->execute();
        if($stmt->rowCount() > 0) {
      ...
      <?php         
        $stmt = $DB_con->prepare('SELECT * FROM social WHERE social_id_user = 58 ORDER BY social_id DESC LIMIT 4');
        $stmt->execute();
        if($stmt->rowCount() > 0) {
          while($row=$stmt->fetch(PDO::FETCH_ASSOC)) {
            extract($row);
  ...
  <header id="header">
    ...
    <div id="logo" class="pull-left" style="margin-right: 25px;">
        <?php        
          $stmt = $DB_con->prepare('SELECT * FROM sobrefran WHERE sobref_id_user = 58 ORDER BY sobref_id ASC LIMIT 1');
          $stmt->execute();
          if($stmt->rowCount() > 0) {
            while($row=$stmt->fetch(PDO::FETCH_ASSOC)) {
              extract($row);
...
<!--==========================
    ...
    <section id="portfolio" class="wow fadeInUp">
      ...
      <div class="container">
        <div class="row no-gutters">
          <?php         
            $stmt = $DB_con->prepare('SELECT * FROM obras_imagens WHERE id_image_user = 58 ORDER BY image_id ASC LIMIT 12');
            $stmt->execute();
            if($stmt->rowCount() > 0) {
              while($row=$stmt->fetch(PDO::FETCH_ASSOC)) {
                extract($row);
    ...
    <section id="team" class="wow fadeInUp">
      <div class="container">
        ...
        <div class="row">
          <?php         
            $stmt = $DB_con->prepare('SELECT * FROM equipe WHERE equipe_id_user = 58 ORDER BY equipe_id ASC LIMIT 12');
            $stmt->execute();
            if($stmt->rowCount() > 0) {
              while($row=$stmt->fetch(PDO::FETCH_ASSOC)) {
                extract($row);
    ...
    <section id="testimonials" class="wow fadeInUp">
      <div class="container">
        ...
        <div class="row">
          <?php         
            $stmt = $DB_con->prepare('SELECT * FROM depoimentos WHERE depoimento_id_user = 58 ORDER BY depoimento_id ASC LIMIT 12');
            $stmt->execute();
            if($stmt->rowCount() > 0) {
              while($row=$stmt->fetch(PDO::FETCH_ASSOC)) {
                extract($row);
